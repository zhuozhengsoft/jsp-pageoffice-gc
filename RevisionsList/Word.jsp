...
<script type="text/javascript">
    ...
    function refreshList() {
        ...
        for (i = 1; i <=poapplication.ActiveDocument.Revisions.Count; i++) {
            var str = "";
            str = str + poapplication.ActiveDocument.Revisions.Item(i).Author;
            var revisionDate = poapplication.ActiveDocument.Revisions.Item(i).Date;
            //转换为标准时间
            str = str + " " + dateFormat(revisionDate, "yyyy-MM-dd HH:mm:ss");

            if (poapplication.ActiveDocument.Revisions.Item(i).Type == "1") {
                str = str + ' 插入：' + poapplication.ActiveDocument.Revisions.Item(i).Range.Text;
            }
            else if (poapplication.ActiveDocument.Revisions.Item(i).Type == "2") {
                str = str + ' 删除：' + poapplication.ActiveDocument.Revisions.Item(i).Range.Text;
            }
            else {
                str = str + ' 调整格式或样式。';
            }
    ...
    function goToRevision(index) {
       poapplication.ActiveDocument.Revisions.Item(index).Range.Select();
    }
